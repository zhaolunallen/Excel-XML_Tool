...
public:
	...
	//通过序号设置某个shell的名称
	void setSheetName(long tableID, LPCTSTR newName);
    //通过一个sheetName创建一个新的sheet 
    bool addNewSheet(LPCTSTR newSheetName);

    //通过一个sheetName使用某个sheet 
    void useSheet(CString newSheetName);
	//通过一个sheetName删除某个sheet 
